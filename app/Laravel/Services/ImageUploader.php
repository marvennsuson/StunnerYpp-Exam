...
class ImageUploader {
	...
	*@return array
	*/
	public static function upload($file, $image_directory = "uploads", $storage = "file"){
		...
		switch (Str::lower($storage)) {
			case 'file':
				...
				Image::make("{$path_directory}/{$filename}")->interlace()
					->widen(640)->save("{$resized_directory}/{$filename}",80);
				// Image::make("{$path_directory}/{$filename}")->interlace()->crop($thumbnail['width'],$thumbnail['height'])->save("{$thumb_directory}/{$filename}",95);
				Image::make("{$path_directory}/{$filename}")->interlace()->widen(320)->save("{$thumb_directory}/{$filename}",80);
